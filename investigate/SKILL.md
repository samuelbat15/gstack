...
description: |
  ...
  analyze, hypothesize, implement. Iron Law: always find the root cause before
  writing any fix. Once the root cause is confirmed, fix it with the smallest
  possible change.
  Use when asked to "debug this", "fix this bug", "why is this broken",
  "investigate this error", or "root cause analysis".
  Always invoke this skill instead of directly modifying code when the user
  reports errors such as 500 errors, stack traces, broken features, or
  troubleshooting scenarios like "it was working yesterday". (gstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
**Step 1: Find the root cause. Step 2: Fix only the root cause.**

These two steps are sequential, not conflicting. The law prohibits jumping to Step 2 without completing Step 1. Once root cause is confirmed, implement the smallest fix that eliminates it — not a workaround, not a symptom patch.

Fixing symptoms creates whack-a-mole debugging. Every fix without root cause makes the next bug harder to find.
...
2. **Minimal diff:** Limit changes to the smallest number of files and lines necessary to resolve the root cause. Do not refactor adjacent code unless it is directly causing the bug.
